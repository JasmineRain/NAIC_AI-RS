...
class MultiRendLoss(nn.Module):
    ...
    def forward(self, output, mask):
        ...
        rend1 = stage1[1].squeeze()
        gt_points1 = sampling_features(mask, stage1[0], mode='bilinear').squeeze().argmax(dim=1)
        # print(rend1.shape, gt_points1.shape)
        point_loss1 = F.cross_entropy(rend1, gt_points1)

        rend2 = stage2[1].squeeze()
        gt_points2 = sampling_features(mask, stage2[0], mode='bilinear').squeeze().argmax(dim=1)
        point_loss2 = F.cross_entropy(rend2, gt_points2)

        rend3 = stage3[1].squeeze()
        gt_points3 = sampling_features(mask, stage3[0], mode='bilinear').squeeze().argmax(dim=1)
        point_loss3 = F.cross_entropy(rend3, gt_points3)

        rend4 = stage4[1].squeeze()
        gt_points4 = sampling_features(mask, stage4[0], mode='bilinear').squeeze().argmax(dim=1)
        point_loss4 = F.cross_entropy(rend4, gt_points4)

        rend5 = stage5[1].squeeze()
        gt_points5 = sampling_features(mask, stage5[0], mode='bilinear').squeeze().argmax(dim=1)
