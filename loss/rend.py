...
class RendLoss(nn.Module):
    ...
    def forward(self, output, mask):
        ...
        gt_points = sampling_features(mask, output['points'], mode='bilinear', align_corners=True).squeeze().argmax(dim=1)
        mask = mask.argmax(dim=1)
        seg_loss = F.cross_entropy(pred, mask)
        # print(pred.shape, mask.shape)
        # print(output['rend'].squeeze().shape, gt_points.shape)
        point_loss = F.cross_entropy(output['rend'].squeeze(), gt_points)
